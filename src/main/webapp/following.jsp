...
<%
    ...
    try {
        ...
        // SQL query to get the users the specified user is following
...
<head>
    ...
    <title><%= profileUsername %>'s Following</title>
    <style>
        body {
...
<body>
    ...
    <div class="container">
        <h2><%= profileUsername %>'s Following</h2>
        <% 
        boolean hasFollowing = false;
        while (rs.next()) {
            hasFollowing = true;
            String followingUsername = rs.getString("username"); // Get the following user's username
            String followingName = rs.getString("name");
            ...
            followStatusStmt.setString(2, followingUsername);
        ...
        %>
            <div class="user">
                ...
                <span>
                    <a href="profile.jsp?username=<%= followingUsername %>" style="color: gold; text-decoration: none;">
                        <%= followingName %>
                    </a>
                </span>

                <%-- Show the follow button only if the user is not the logged-in user --%>
                <% if (!followingUsername.equals(loggedInUsername)) { %>
                    <form action="followAction.jsp" method="post" style="display: inline;">
                        <input type="hidden" name="followUsername" value="<%= followingUsername %>">
